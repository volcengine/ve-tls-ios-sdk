...
__attribute__ ((noinline)) void Vebf_warnBlockingOperationOnMainThread() {
    NSLog(@"Warning: A long-running operation is being executed on the main thread. \n"
          " Break on Vebf_warnBlockingOperationOnMainThread() to debug.");
}

NSString *const VeTaskErrorDomain = @"bolts";
NSInteger const kVeMultipleErrorsError = 80175001;

NSString *const VeTaskMultipleErrorsUserInfoKey = @"errors";
...
+ (instancetype)taskForCompletionOfAllTasks:(nullable NSArray<VeTask *> *)tasks {
    ...
    VeTaskCompletionSource *tcs = [VeTaskCompletionSource taskCompletionSource];
    for (VeTask *task in tasks) {
        [task continueWithBlock:^id(VeTask *t) {
            if (t.error) {
                @synchronized (lock) {
            ...
            if (OSAtomicDecrement32Barrier(&total) == 0) {
                if (errors.count > 0) {
                    if (errors.count == 1) {
                        ...
                        NSError *error = [NSError errorWithDomain:VeTaskErrorDomain
                                                             code:kVeMultipleErrorsError
                                                         userInfo:@{ VeTaskMultipleErrorsUserInfoKey: errors }];
                        tcs.error = error;
                    }
                } else if (cancelled > 0) {
                    [tcs cancel];
...
+ (instancetype)taskForCompletionOfAllTasksWithResults:(nullable NSArray<VeTask *> *)tasks {
    return [[self taskForCompletionOfAllTasks:tasks] continueWithSuccessBlock:^id(VeTask * __unused task) {
...
+ (instancetype)taskForCompletionOfAnyTask:(nullable NSArray<VeTask *> *)tasks
{
    ...
    VeTaskCompletionSource *source = [VeTaskCompletionSource taskCompletionSource];
    for (VeTask *task in tasks) {
        [task continueWithBlock:^id(VeTask *t) {
            if (t.error != nil) {
                @synchronized(lock) {
...
+ (instancetype)taskFromExecutor:(VeExecutor *)executor withBlock:(nullable id (^)(void))block {
    return [[self taskWithResult:nil] continueWithExecutor:executor withBlock:^id(VeTask *task) {
        return block();
    }];
}
...
- (VeTask *)continueWithExecutor:(VeExecutor *)executor withBlock:(VeContinuationBlock)block {
...
- (VeTask *)continueWithExecutor:(VeExecutor *)executor
    ...
    dispatch_block_t executionBlock = ^{
        ...
        if ([result isKindOfClass:[VeTask class]]) {

            id (^setupWithTask) (VeTask *) = ^id(VeTask *task) {
            ...
            VeTask *resultTask = (VeTask *)result;
...
- (VeTask *)continueWithBlock:(VeContinuationBlock)block {
    return [self continueWithExecutor:[VeExecutor defaultExecutor] block:block cancellationToken:nil];
}

- (VeTask *)continueWithBlock:(VeContinuationBlock)block cancellationToken:(nullable VeCancellationToken *)cancellationToken {
    return [self continueWithExecutor:[VeExecutor defaultExecutor] block:block cancellationToken:cancellationToken];
}

- (VeTask *)continueWithExecutor:(VeExecutor *)executor
                withSuccessBlock:(VeContinuationBlock)block {
...
- (VeTask *)continueWithExecutor:(VeExecutor *)executor
    ...
    return [self continueWithExecutor:executor block:^id(VeTask *task) {
...
- (VeTask *)continueWithSuccessBlock:(VeContinuationBlock)block {
    return [self continueWithExecutor:[VeExecutor defaultExecutor] successBlock:block cancellationToken:nil];
}

- (VeTask *)continueWithSuccessBlock:(VeContinuationBlock)block cancellationToken:(nullable VeCancellationToken *)cancellationToken {
    return [self continueWithExecutor:[VeExecutor defaultExecutor] successBlock:block cancellationToken:cancellationToken];
}

#pragma mark - Syncing Task (Avoid it)

- (void)warnOperationOnMainThread {
    Vebf_warnBlockingOperationOnMainThread();
}
